...
def Init():
    if config.get('token') == "token-here":
        ...
        token = config.get('token')
        try:
            Xanarchy.run(token, bot=False, reconnect=True)
            os.system(f'title (Xanarchy Selfbot) - Version {SELFBOT.__version__}')
...
async def on_message_edit(before, after):
    await Xanarchy.process_commands(after)
...
async def on_message(message):
    ...
    if 'privnote.com' in message.content:
        if privnote_sniper == True:
            ...
            return
    await Xanarchy.process_commands(message)
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'Xanarchy-Was-Here@Fuckyou.com';
...
@Xanarchy.command(aliases=['shorteen'])
...
@Xanarchy.command(name='ebay-view', aliases=['ebay-view-bot', 'ebayviewbot', 'ebayview'])
...
@Xanarchy.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@Xanarchy.command(aliases=['pfp', 'avatar'])
...
@Xanarchy.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
              await Xanarchy.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Xanarchy.command(name='set-pfp', aliases=['setpfp', 'pfpset'])
...
@Xanarchy.command(aliases=['dong', 'penis'])
...
@Xanarchy.command(aliases=['changehypesquad'])
...
@Xanarchy.command(aliases=['fakeconnection', 'spoofconnection'])
...
@Xanarchy.command(aliases=['tokinfo', 'tdox'])
...
async def copy(ctx): # b'\xfc'
    await ctx.message.delete()
    await Xanarchy.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Xanarchy.guilds:
...
async def destroy(ctx): # b'\xfc'
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="https://Xanarchy.wtf",
...
async def dm(ctx, user : discord.Member, *, message): # b'\xfc'
    await ctx.message.delete()
    user = Xanarchy.get_user(user.id)
    if ctx.author.id == Xanarchy.user.id:
        return
    else:
        try:
            await user.send(message) 
...
@Xanarchy.command(aliases=['rainbow-role'])
...
@Xanarchy.command(aliases=['slots', 'bet'])
...
@Xanarchy.command(name='auto-bump', aliases=['bump'])
...
@Xanarchy.command(aliases=['guildpfp'])
...
@Xanarchy.command(name='backup-f', aliases=['friendbackup', 'friend-backup', 'backup-friends', 'backupf'])
...
@Xanarchy.command(aliases=['bitcoin'])
...
@Xanarchy.command(aliases=['ethereum'])
...
@Xanarchy.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Xanarchy.command()
async def smallboobs(ctx): # b'\xfc'
    await ctx.message.delete()
...
@Xanarchy.command(aliases=['proxy'])
...
async def purge(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Xanarchy.user).map(lambda m: m):
        try:
           await message.delete()
        except:
            pass

@Xanarchy.command(name='group-leaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def _group_leaver(ctx): # b'\xfc'
    await ctx.message.delete()
    for channel in Xanarchy.private_channels:
...
async def iplist(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://www.exxploiting.com/ip/'
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await Xanarchy.change_presence(activity=game)
...
@Xanarchy.command(aliases=['markasread', 'ack'])
async def read(ctx): # b'\xfc'
    await ctx.message.delete()
    for guild in Xanarchy.guilds:
        await guild.ack()
...
async def fuckoff(ctx): # b'\xfc'
    await ctx.message.delete()
    foff = 'https://imgur.com/sG9VOKb'
    await ctx.send(foff)

@Xanarchy.command()
async def clown(ctx): # b'\xfc'
    await ctx.message.delete()
    clown = 'https://cdn.discordapp.com/attachments/761422499711025192/763277280935542844/clown_guy.mp4'
...
@Xanarchy.command(name='role-hexcode', aliases=['rolecolor'])
async def _role_hexcode(ctx, *, role: discord.Role): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(f"{role.name} : {role.color}")

@Xanarchy.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
...
@Xanarchy.command(aliases=['clearconsole', 'consoleclear'])
...
@Xanarchy.command(name='gmail-bomb', aliases=['gmail-bomber', 'gmailbomb', 'email-bomber', 'emailbomber'])
